...
export function ActivityClient({ initialVideos }: ActivityClientProps) {
  const [filter, setFilter] = useState("all");
  const api = useApi();

  const { data: videos, isLoading, isRefetching } = useQuery({
    queryKey: ["videos"],
    queryFn: async () => {
  ...
  const showSkeleton = isLoading && activity.length === 0;
