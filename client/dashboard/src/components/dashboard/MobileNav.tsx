...
import { LayoutDashboard, HardDrive, Video, MonitorPlay, Key, Gauge, CreditCard, Users, ScrollText, BookOpen, Settings } from "lucide-react";
...
export function MobileNav() {
  ...
  const navItems = [
    ...
    { name: "Convert", href: "/dashboard/media-convert", icon: MonitorPlay, match: (p: string) => p.startsWith("/dashboard/media-convert") },
    ...
    { name: "Activity", href: "/dashboard/activity", icon: ScrollText, match: (p: string) => p.startsWith("/dashboard/activity") },
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 border-t border-borderSubtle bg-base/95 backdrop-blur-md pb-safe z-50">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`w-1 h-1 rounded-full ${isActive ? "bg-accent-motion" : "bg-transparent"}`} />
            </Link>
          );
        })}
      </nav>
